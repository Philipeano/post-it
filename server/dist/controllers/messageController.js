...
var messageModel = _index2.default.Message;
var notificationModel = _index2.default.Notification;
        ...
        groupModel.findById(req.params.groupId).then(function (matchingGroup) {
          if (matchingGroup) {
            membershipModel.findOne({
              ...
              if (membership) {
                messageModel.sync().then(function () {
                  messageModel.create({
                    groupId: req.params.groupId,
    ...
    value: function sendNotifications(req, res, postedMessage) {
      membershipModel.findAll({
        ...
        if (memberships) {
          ...
          notificationModel.sync().then(function () {
            notificationModel.bulkCreate(notificationsList).then(function () {
              res.status(201).json({
                message: 'Message posted to group successfully!',
    ...
    value: function getMessagesFromGroup(req, res) {
      ...
      if (errorMessage.trim() !== '') res.status(400).json({ message: errorMessage });else {
        membershipModel.findOne({
          ...
          if (membership) {
            messageModel.findAll({ where: { groupId: req.params.groupId } }).then(function (messages) {
      ...
      if (errorMessage.trim() !== '') res.status(400).json({ message: errorMessage });else {
        membershipModel.findOne({
          where: { groupId: req.params.groupId,
            memberId: req.session.user.id } }).then(function (membership) {
          if (membership) {
            messageModel.findOne({ where: { groupId: req.params.groupId,
                id: req.params.messageId,
                senderId: req.session.user.id } }).then(function (message) {
              if (message) {
                messageModel.update({ content: req.body.content }, { where: { id: req.params.messageId },
                  returning: true,
                  plain: true
      ...
      if (errorMessage.trim() !== '') res.status(400).json({ message: errorMessage });else {
        membershipModel.findOne({
          where: { groupId: req.params.groupId,
            memberId: req.session.user.id } }).then(function (membership) {
          if (membership) {
            messageModel.findAll({ where: { groupId: req.params.groupId,
                id: req.params.messageId,
                senderId: req.session.user.id } }).then(function (messages) {
              if (messages) {
                messageModel.destroy({ where: { id: req.params.messageId } }).then(function () {
